...
import {
  ...
  MD3LightTheme as DLightDT,
  MD3DarkTheme as DDarkDt,
...
/* adapt theme foe reatc navigation */
const { LightTheme, DarkTheme } = adaptNavigationTheme({light: DefaultTheme, dark: DT});
/* cretae context to enable dmode switch */
const [useDarkMode, DarkModeProvider] = createTypeContext<{
...
export default function ThemeProvider ({ children }: { children: React.ReactNode }) {
  ...
  const userDkMode = useColorScheme();
  const [darkMode, setDarkMode] = useState(userDkMode === "dark" ? true : false);
  /* memoize props */
  const darkModeProps = useMemo(() => {
    return  {
      setDarkMode,
      darkMode
    }
  ...
  return (
    <PaperProvider theme={darkMode ? cuDarkTheme : cuLightTheme}>
    <NavigationContainer theme={darkMode ? DarkTheme : LightTheme}>
    <DarkModeProvider value={darkModeProps}>
